...
### Game error alert feature
In the event something goes wrong with the game, in particular with the fetch requests, then the user will receive a sweetalert2 alert, indicating to a user that a game error has occurred. The alert, which cannot be closed, in order to prevent the user playing the broken game, prompts the user to first try and refresh the page, and if the same alert appears again, to try again later. 
